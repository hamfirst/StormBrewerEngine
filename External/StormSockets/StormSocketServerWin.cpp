...
namespace StormSockets
{
	...
	void StormSocketServerWin::AcceptNewConnection()
	{
		...
		StormSocketServerConnectionId connection_id = AllocateConnection(m_AcceptSocket, sin.sin_addr.S_un.S_addr, sin.sin_port);
	...
	void StormSocketServerWin::ProcessNewData(StormSocketServerConnectionId connection_id, int bytes_received)
	{
    printf("Got %d bytes\n", bytes_received);
	...
	bool StormSocketServerWin::ProcessReceivedData(StormSocketServerConnectionId connection_id)
	{
		// Lock the receiver
		int old_val = 0;
	...
	void StormSocketServerWin::PrepareToRecv(StormSocketServerConnectionId connection_id)
	{
    WSABUF buffer_set[2];
	...
	void StormSocketServerWin::IOThreadMain()
	{
		...
		while (m_ThreadStopRequested == false)
		{
			...
			if (success)
			{
				...
				StormSocketServerConnectionId connection_id;
            ...
            if (writer.m_Encrypt && m_UseSSL)
            {
              ...
              if (writer.m_Encrypt && m_UseSSL)
              {
                ...
                writer = encrypted;
						...
						//Console.WriteLine("Sending {0} bytes to {1}", total_send_length, connection_id.Index);
	...
	void StormSocketServerWin::ReleaseSendQueue(StormSocketServerConnectionId connection_id, int connection_gen)
	{
		StormMessageWriter writer;
  ...
  StormMessageWriter StormSocketServerWin::EncryptWriter(StormSocketServerConnectionId connection_id, StormMessageWriter & writer)
  {
#ifdef USE_WINSEC
  ...
  void StormSocketServerWin::CloseSocket(StormSocketServerConnectionId id)
  {
  ...
  void StormSocketServerWin::FreeConnectionResources(StormSocketServerConnectionId id)
  {
