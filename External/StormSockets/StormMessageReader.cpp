...
{
	StormSocketDataType::Index StormMessageReader::GetDataType()
	{
		return m_DataType;
	}

	bool StormMessageReader::GetFinalInSequence()
	{
		return m_FinalInSequence;
	}

	void StormMessageReader::Init(StormFixedBlockAllocator * block_allocator, StormFixedBlockAllocator * reader_allocator, StormFixedBlockHandle cur_block,
	...
	void StormMessageReader::SetNextBlock(const StormMessageReader & next_reader)
	{
		m_ReaderAllocator->SetNextBlock(m_PacketHandle, next_reader.m_PacketHandle);
	}

	bool StormMessageReader::InvalidateNext(StormMessageReader & next_reader)
	{
	...
	void StormMessageReader::AddLength(int data_len)
	{
		m_FullDataLen += data_len;
	}

	void StormMessageReader::Advance()
	{
	...
	void StormMessageReader::FreeChain()
	{
	...
	int StormMessageReader::GetDataLength()
	{
		return m_FullDataLen;
	}

	uint8_t StormMessageReader::ReadByte()
	{
	...
	wchar_t StormMessageReader::ReadUTF8Char()
	{
		int b1 = ReadByte();
		if (b1 < 127)
		{
	...
	uint16_t StormMessageReader::ReadInt16()
	{
		int read_offset = m_PacketInfo->m_ReadOffset;
		int data_length = m_PacketInfo->m_DataLength;
	...
	uint32_t StormMessageReader::ReadInt32()
	{
		int read_offset = m_PacketInfo->m_ReadOffset;
		int data_length = m_PacketInfo->m_DataLength;
	...
	uint64_t StormMessageReader::ReadInt64()
	{
		int read_offset = m_PacketInfo->m_ReadOffset;
		int data_length = m_PacketInfo->m_DataLength;
