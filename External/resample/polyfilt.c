...
void polyfiltLM(struct PSFilter* pfilt, struct PSState* pstate,
		const double* signal, const int signalLen, int* nbRead, double* output,
		const int outputLen, int* nbWritten) {
	const int M = pfilt->M;
	const int L = pfilt->L;
	const int K = pfilt->K;
...
void polyfiltM(struct PSFilter* pfilt, struct PSState* pstate,
		const double* signal, const int signalLen, int* nbConsume,
		double* output, const int outputLen, int* nbWritten) {
	const int M = pfilt->M;
	const int K = pfilt->K;
