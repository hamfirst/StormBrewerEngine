...
void RegisterServerObjectOverapCallbacks()
{
  auto visitor = [&](auto f)
  {
    constexpr bool IsOverlap = f.GetFunctionNameHash() == 0xB0B10809; // "OnOverlap"
  ...
  { \
    ...
    { \
      g_ServerObjectOverlapSystemRegister.AddCall([] { \
        g_ServerObjectOverlapSystem.RegisterObjectType(DataClass::TypeIndex, Sprite.GetResource(), COMPILE_TIME_CRC32_STR("MoveBox")); \
      }); \
