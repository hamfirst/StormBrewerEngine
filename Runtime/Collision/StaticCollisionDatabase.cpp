...
uint32_t StaticCollisionDatabase::CheckCollision(const Box & box, uint32_t collision_layer_mask) const
{
  uint32_t result = 0;
...
uint32_t StaticCollisionDatabase::CheckCollisionAny(const Box & box, uint32_t collision_layer_mask) const
{
  uint32_t result = 0;
  for (std::size_t index = 0; index < m_CollisionLayers.size(); index++)
  {
...
uint32_t StaticCollisionDatabase::CheckLineOfSight(const Vector2 & start, const Vector2 & end, uint32_t collision_layer_mask) const
{
  uint32_t result = 0;
...
uint32_t StaticCollisionDatabase::CheckClearance(const Vector2 & pos, uint32_t maximum_clearance, uint32_t collision_layer_mask) const
{
  Box box = { pos, pos };
...
Optional<Box> StaticCollisionDatabase::PushMapCollision(std::size_t map_id, std::vector<std::vector<Box>> && collision_boxes)
{
  Optional<Box> bounds;
...
void StaticCollisionDatabase::RemoveMapCollision(std::size_t map_id)
{
  for (auto & elem : m_CollisionLayers)
  {
