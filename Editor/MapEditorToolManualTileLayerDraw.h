...
{
public:
  MapEditorToolManualTileLayerDraw(MapEditor & map_editor, int layer_index, bool animation, uint64_t frame_id);
  ...
  Optional<MapTile> GetPreviewTile(const Vector2 & pos);
  Optional<MapAnimatedTile> GetPreviewAnimation(const Vector2 & pos);
...
protected:
  ...
  std::vector<MapAnimatedTile> m_DrawAnims;
};
