...
function StormWebrtcCreateConnection(index, ipaddr_ptr, fingerprint_ptr, port, inc_types_ptr, inc_types_len, out_types_ptr, out_types_len) {
    ...
    for(var stream = 0; stream < out_types_len; stream++) {
        ...
        var out_channel = connection.createDataChannel(stream, channel_options);
    ...
    _webrtc_array[index].connection.ondatachannel = function(event) {
        ...
        let stream_index = floor(channel.id / 2);
    ...
    _webrtc_array[index].connection.onconnectionstatechange = function(event) {
    ...
    _webrtc_array[index].connection.createOffer().then(function(offer) { 
        ...
        connection.setLocalDescription(offer).then(function() {
            ...
            connection.setRemoteDescription(desc).then(function() {
                var ice_candidate = {
                    'candidate': "a=candidate:0 1 UDP 1 "+ipaddr_ptr+" "+port+" typ host",
                    'sdpMid': "data",
                    'sdpMLineIndex': 0
                };
