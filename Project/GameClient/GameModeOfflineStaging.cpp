...
void GameModeOfflineStaging::OnAssetsLoaded()
{
  ...
  m_Start.Emplace(m_UIManager, "start", nullptr, Box::FromFrameCenterAndSize(Vector2(half_res.x - 85, 30 - half_res.y), Vector2(150, 25)), "Start");
  m_Start->SetOnClickCallback([this] { Start(); });

  m_Back.Emplace(m_UIManager, "back", nullptr, Box::FromFrameCenterAndSize(Vector2(half_res.x - 85, 30 - half_res.y), Vector2(150, 25)), "Back", nullptr, true);
  m_Back->SetOnClickCallback([this] { Back(); });

  m_StagingUI = m_UIManager.AllocateElementFromDef("staging", *g_GameClientStagingUI.GetData());
