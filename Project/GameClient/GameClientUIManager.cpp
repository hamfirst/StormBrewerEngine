  ...
  m_DisabledPopup(false)
{
  ...
  m_QuitPopup.Emplace(m_UIManager, "quit", nullptr, Box::FromFrameCenterAndSize(Vector2(0, 0), Vector2(250, 100)), "Quit To Main Menu?");
...
void GameClientUIManager::ShowTutorial()
{
  ...
  m_Tutorial = m_UIManager.AllocateElementFromDef("tutorial", *g_GameTutorialUI.GetData());
  m_Tutorial->SetInput(COMPILE_TIME_CRC32_STR("Low"), 0.0f);

  auto okay_box = Box::FromFrameCenterAndSize(Vector2(0, -40), Vector2(150, 30));
  m_TutorialOkay.Emplace(m_UIManager, "tutorial_okay", nullptr, okay_box, "Okay");
...
void GameClientUIManager::TogglePopup()
{
  ...
  else if(m_DisabledPopup == false)
  {
    ...
    m_FullscreenButton.Emplace(m_UIManager, "fullscreen", nullptr, half_res - Vector2(20, 20), m_GameContainer.GetWindow());
