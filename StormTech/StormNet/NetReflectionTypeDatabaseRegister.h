...
void NetInitializeTypeRegistration(NetTypeRegistrationInfo & reg)
{
  ...
  reg.m_TypeInfo.m_HeapCreateCopy = [](const void * src, void * alloc) -> void * { auto src_ptr = static_cast<const RegType *>(src); auto alloc_ptr = static_cast<NetPoolAllocator<RegType> *>(alloc); return alloc_ptr->Allocate(*src_ptr); };
  reg.m_TypeInfo.m_HeapCreateMove = [](void * src, void * alloc) -> void * { auto src_ptr = static_cast<RegType *>(src); auto alloc_ptr = static_cast<NetPoolAllocator<RegType> *>(alloc); return alloc_ptr->Allocate(std::move(*src_ptr)); };
