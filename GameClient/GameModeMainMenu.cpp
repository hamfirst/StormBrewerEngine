...
void GameModeMainMenu::OnAssetsLoaded()
{
  ...
  m_PlayOnline.Emplace(m_UIManager, "playonline", nullptr, 
    Box::FromFrameCenterAndSize(Vector2(-half_res.x + 75, 125 - half_res.y), Vector2(120, 25)), "Play Online", &container.GetClientGlobalResources().UISoundEffects);
  m_PlayOnline->SetOnClickCallback([this] { PlayOnline(); });

  m_Tutorial.Emplace(m_UIManager, "tutorial", nullptr,
    Box::FromFrameCenterAndSize(Vector2(-half_res.x + 75, 100 - half_res.y), Vector2(120, 18)), "Tutorial", &container.GetClientGlobalResources().UISoundEffects);
  m_Tutorial->SetOnClickCallback([this] { Tutorial(); });

  m_PlayOffline.Emplace(m_UIManager, "playoffline", nullptr, 
    Box::FromFrameCenterAndSize(Vector2(-half_res.x + 75, 80 - half_res.y), Vector2(120, 18)), "Local Multiplayer", &container.GetClientGlobalResources().UISoundEffects);
  m_PlayOffline->SetOnClickCallback([this] { PlayOffline(); });

  m_PlaySingleplayer.Emplace(m_UIManager, "playsingleplayer", nullptr,
    Box::FromFrameCenterAndSize(Vector2(-half_res.x + 75, 60 - half_res.y), Vector2(120, 18)), "Single Player", &container.GetClientGlobalResources().UISoundEffects);
  m_PlaySingleplayer->SetOnClickCallback([this] { PlaySingleplayer(); });

  m_CreateOnlineMatch.Emplace(m_UIManager, "creategame", nullptr,
                             Box::FromFrameCenterAndSize(Vector2(-half_res.x + 75, 35 - half_res.y), Vector2(120, 18)), "Create Private Match", &container.GetClientGlobalResources().UISoundEffects);
  m_CreateOnlineMatch->SetOnClickCallback([this] { CreatePrivateMatch(); });

  m_JoinOnlineMatch.Emplace(m_UIManager, "joingame", nullptr,
                             Box::FromFrameCenterAndSize(Vector2(-half_res.x + 75, 15 - half_res.y), Vector2(120, 18)), "Join Private Match", &container.GetClientGlobalResources().UISoundEffects);
